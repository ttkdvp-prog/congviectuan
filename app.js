...
function handleModalGroupChange(selectedGroup) {
  ...
  if (!selectedGroup) {
    // No group selected: show all users from cache
    populateDropdownFromCache('');
    return;
  }

  // Show loading indicator
  if (chuTriInput) chuTriInput.placeholder = '⏳ Đang tải nhân viên...';
  if (phoiHopInput) phoiHopInput.placeholder = '⏳ Đang tải nhân viên...';
  ...
  if (appState.isGAS) {
    try {
      google.script.run
        .withSuccessHandler(function(res) {
          applyFreshTeamMembers(res, selectedGroup);
          if (chuTriInput) chuTriInput.placeholder = 'Tất cả nhân viên trong tổ...';
          if (phoiHopInput) phoiHopInput.placeholder = 'Tất cả nhân viên trong tổ...';
        })
        .withFailureHandler(function(err) {
          console.warn('[handleModalGroupChange] GAS failed, using cache:', err);
          populateDropdownFromCache(selectedGroup);
          if (chuTriInput) chuTriInput.placeholder = 'Tất cả nhân viên trong tổ...';
          if (phoiHopInput) phoiHopInput.placeholder = 'Tất cả nhân viên trong tổ...';
        })
        .getTeamMembers(selectedGroup);
    } catch(e) {
      populateDropdownFromCache(selectedGroup);
      if (chuTriInput) chuTriInput.placeholder = 'Tất cả nhân viên trong tổ...';
      if (phoiHopInput) phoiHopInput.placeholder = 'Tất cả nhân viên trong tổ...';
    ...
    const url = fetchUrl + '?action=getTeamMembers&group=' + encodeURIComponent(selectedGroup) + '&t=' + new Date().getTime();
    fetch(url)
      .then(res => res.json())
      .then(res => {
        if (res && res.success && res.members && res.members.length > 0) {
          ...
          console.warn('[handleModalGroupChange] Empty response, using cache. Response:', JSON.stringify(res).substring(0, 300));
          populateDropdownFromCache(selectedGroup);
        }
        if (chuTriInput) chuTriInput.placeholder = 'Tất cả nhân viên trong tổ...';
        if (phoiHopInput) phoiHopInput.placeholder = 'Tất cả nhân viên trong tổ...';
      ...
      .catch(err => {
        console.warn('[handleModalGroupChange] Fetch failed, using cache:', err);
        populateDropdownFromCache(selectedGroup);
        if (chuTriInput) chuTriInput.placeholder = 'Tất cả nhân viên trong tổ...';
        if (phoiHopInput) phoiHopInput.placeholder = 'Tất cả nhân viên trong tổ...';
      });
  } else {
    // No API URL: use cache only
    populateDropdownFromCache(selectedGroup);
  }
}

// Populate dropdown from cached appState data (fallback when API unavailable)
...
function applyFreshTeamMembers(res, groupName) {
  ...
  // Add all members from fresh response
  if (res.members) {
  ...
  // Update dropdown data with ONLY fresh data (no cache mixing)
  appState.dropdownData.chutri = sortedUsers;
  appState.dropdownData.phoihop = sortedUsers;
  
  console.log('[applyFreshTeamMembers] ✅ Updated dropdowns with', sortedUsers.length, 'fresh members for', groupName);
  
  // Refresh any open dropdown immediately
