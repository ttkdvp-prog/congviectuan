  ...
  if (appState.isGAS) {
    ...
    if (fetchUrl) {
      fetch(fetchUrl, {
        method: 'POST',
        headers: { 'Content-Type': 'text/plain' },
        body: JSON.stringify({ action: 'getToTruongSheetData' })
      })
        .then(res => res.json())
        .then(res => {
          if (res && res.success && res.data && res.data.length > 0) {
            appState.totruonggiaoviec = res.data;
            console.log('[loadToTruongBackup] SUCCESS via fetch:', res.data.length, 'records');
            try { localStorage.setItem('TTHT_TOTRUONGGIAOVIEC_CACHE', JSON.stringify(res.data)); } catch(e) {}
            renderActiveTab();
          }
        })
        .catch(err => console.warn('[loadToTruongBackup] Fetch failed:', err));
    }
  }
}
