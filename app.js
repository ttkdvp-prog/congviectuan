...
function onToTruongGroupChange() {
  ...
  if (leaderBanner) {
    if (selGroup && leaderInfo) {
      ...
      const isLanPhuongLeader = cleanKey(displayLeaderName).includes('vuthilanphuong') || cleanKey(displayLeaderName).includes('lanphuong');
      const isLanPhuongSelected = currentUsr && (cleanKey(currentUsr).includes('vuthilanphuong') || cleanKey(currentUsr).includes('lanphuong'));

      if (isLanPhuongLeader || isLanPhuongSelected) {
        roleTitle = 'Tổ phó';
        if (isLanPhuongSelected) {
          displayLeaderName = 'Vũ Thị Lan Phương';
        }
        displayLeaderCode = 'VNPT018275';
      }
  ...
  Array.from(teamUsers).sort().forEach(u => {
    const isLanPhuong = cleanKey(u).includes('vuthilanphuong') || cleanKey(u).includes('lanphuong');
    const label = isLanPhuong ? 'Tổ phó: Vũ Thị Lan Phương (VNPT018275)' : u;
