...
function populateSelects() {
  ...
  if (taskToChuTriSelect) {
    ...
    taskToChuTriSelect.innerHTML = '<option value="">-- Tất cả tổ --</option>';
    Array.from(hostGroups).sort().forEach(g => {
      if (g) taskToChuTriSelect.innerHTML += `<option value="${escapeHtml(g)}">${escapeHtml(g)}</option>`;
    });
...
function getTaskEmpRName(task) {
  if (!task || typeof task !== 'object') return '';

  const exactR = getExactValueByKeyPattern(task, ['Tên NV (R)', 'Tên NV R', 'tennvr']);
  if (exactR) return exactR;
  if (task['Tên NV (R)']) return String(task['Tên NV (R)']).trim();

  const exactNV = getExactValueByKeyPattern(task, ['Tên NV', 'tennv']);
  if (exactNV) return exactNV;

  const leaderName = getTaskLeaderName(task);
  const chuTri = task['Người chủ trì'] || task['Người thực hiện'] || '';
  if (chuTri && cleanKey(chuTri) !== cleanKey(leaderName)) return String(chuTri).trim();

  const nameA = task['Tên NV (A)'] || '';
  if (nameA && cleanKey(nameA) !== cleanKey(leaderName)) return String(nameA).trim();

  return chuTri || nameA || '';
}

function getTaskEmpRCode(task) {
  if (!task || typeof task !== 'object') return '';

  const exactR = getExactValueByKeyPattern(task, ['Mã NV (R)', 'Mã NV R', 'manvr']);
  if (exactR) return exactR;
  if (task['Mã NV (R)']) return String(task['Mã NV (R)']).trim();

  const exactCode = getExactValueByKeyPattern(task, ['Mã NV', 'manv']);
  if (exactCode) return exactCode;

  const leaderCode = getTaskLeaderCode(task);
  const codeA = task['Mã NV (A)'] || '';
  if (codeA && cleanKey(codeA) !== cleanKey(leaderCode)) return String(codeA).trim();

  return '';
}

function getTaskEmpCName(task) {
  if (!task || typeof task !== 'object') return '';

  const exactC = getExactValueByKeyPattern(task, ['Tên NV (C)', 'Tên NV C', 'tennvc']);
  if (exactC) return exactC;
  if (task['Tên NV (C)']) return String(task['Tên NV (C)']).trim();

  const exactPhoiHop = getExactValueByKeyPattern(task, ['tên người phối hợp (R)', 'Tên người phối hợp (R)', 'Tên người phối hợp', 'tennguoiphoihopr', 'tennguoiphoihop', 'nguoiphoihop']);
  if (exactPhoiHop) return exactPhoiHop;

  return task['tên người phối hợp (R)'] || task['Tên người phối hợp (R)'] || task['Người phối hợp'] || '';
}

function getTaskEmpCCode(task) {
  if (!task || typeof task !== 'object') return '';

  const exactC = getExactValueByKeyPattern(task, ['Mã NV (C)', 'Mã NV C', 'manvc']);
  if (exactC) return exactC;
  if (task['Mã NV (C)']) return String(task['Mã NV (C)']).trim();

  const exactPhoiHop = getExactValueByKeyPattern(task, ['mã NV người phối hợp (R)', 'Mã NV người phối hợp (R)', 'manguoiphoihopr', 'manvnguoiphoihopr']);
  if (exactPhoiHop) return exactPhoiHop;
...
function renderToTruongTaskList() {
  ...
  let filtered = toTruongTasks.filter(t => {
    ...
    if (usrClean) {
      const leaderNameClean = cleanKey(getTaskLeaderName(t) || t['Tên người giao việc'] || t['Tên tổ trưởng'] || '');
      const leaderCodeClean = cleanKey(getTaskLeaderCode(t) || t['Mã người giao việc'] || t['Mã NV tổ trưởng'] || '');

      const empNameClean = cleanKey(getTaskEmpName(t) || t['Tên NV (A)'] || t['Người chủ trì'] || '');
      const empCodeClean = cleanKey(getTaskEmpCode(t) || t['Mã NV (A)'] || '');

      const phoiHopNameClean = cleanKey(getTaskPhoiHopName(t) || t['tên người phối hợp (R)'] || t['Người phối hợp'] || '');
      const phoiHopCodeClean = cleanKey(getTaskPhoiHopCode(t) || t['mã NV người phối hợp (R)'] || '');
      ...
      // 1. Leader match (người giao việc)
      ...
      // 2. Assignee match (Tên NV A)
      if (!match && empNameClean && (empNameClean.includes(usrClean) || usrClean.includes(empNameClean))) {
        match = true;
      }
      if (!match && empCodeClean && (empCodeClean.includes(usrClean) || usrClean.includes(empCodeClean))) {
        match = true;
      }

      // 3. Collaborator match (tên người phối hợp R)
      if (!match && phoiHopNameClean && (phoiHopNameClean.includes(usrClean) || usrClean.includes(phoiHopNameClean))) {
        match = true;
      }
      if (!match && phoiHopCodeClean && (phoiHopCodeClean.includes(usrClean) || usrClean.includes(phoiHopCodeClean))) {
        match = true;
      }
