...
function getTaskEmpRName(task) {
  if (!task || typeof task !== 'object') return '';
  const exact = getExactValueByKeyPattern(task, ['Tên NV (R)', 'Tên NV', 'tennvr', 'tennv']);
  if (exact) return exact;
  return task['Tên NV (R)'] || task['Tên NV'] || '';
}

function getTaskEmpRCode(task) {
  if (!task || typeof task !== 'object') return '';
  const exact = getExactValueByKeyPattern(task, ['Mã NV (R)', 'Mã NV', 'manvr', 'manv']);
  if (exact) return exact;
  return task['Mã NV (R)'] || task['Mã NV'] || '';
}

function getTaskEmpCName(task) {
  if (!task || typeof task !== 'object') return '';
  const exact = getExactValueByKeyPattern(task, ['Tên NV (C)', 'tên người phối hợp (R)', 'Tên người phối hợp (R)', 'tennvc', 'tennguoiphoihopr']);
  if (exact) return exact;
  return task['Tên NV (C)'] || task['tên người phối hợp (R)'] || task['Tên người phối hợp (R)'] || '';
}

function getTaskEmpCCode(task) {
  if (!task || typeof task !== 'object') return '';
  const exact = getExactValueByKeyPattern(task, ['Mã NV (C)', 'mã NV người phối hợp (R)', 'Mã NV người phối hợp (R)', 'manvc', 'manguoiphoihopr']);
  if (exact) return exact;
  return task['Mã NV (C)'] || task['mã NV người phối hợp (R)'] || task['Mã NV người phối hợp (R)'] || '';
}
