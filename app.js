...
function getToTruongTasks() {
  ...
  const isFromToTruongSheet = (source === appState.totruonggiaoviec);

  return source.map(t => {
    const title = t['Tiêu đề'] || t['Tiêu đề công việc'] || '';
    const desc = t['Mô tả'] || t['Mô tả công việc'] || '';

    const leaderName = getTaskLeaderName(t) || (isFromToTruongSheet ? '' : getTaskAssignee(t));
    const leaderCode = getTaskLeaderCode(t);

    const empRName = getTaskEmpRName(t) || (isFromToTruongSheet ? '' : (getTaskAssignee(t) !== leaderName ? getTaskAssignee(t) : ''));
    const empRCode = getTaskEmpRCode(t);

    const empCName = getTaskEmpCName(t) || (isFromToTruongSheet ? '' : getTaskCollaborator(t));
