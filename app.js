...
function getTaskGroup(task) {
  if (!task) return '';
  let g = task['Tổ chủ trì'] || task['Tổ'] || task['TỔ'] || task['Tên tổ'] || task['To'];
...
function populateSelects() {
  ...
  appState.tasks.forEach(t => {
    ...
    if (tcg) allGroups.add(String(tcg).trim());
  });

  appState.cvluuy.forEach(item => {
    const g = item['Tổ'] || item['TỔ'] || item['Tên tổ'];
    if (g) allGroups.add(String(g).trim());
  });
