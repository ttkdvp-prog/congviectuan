...
// Returns full employee list from Sheet User with selected team prioritized
function getEmployeesByGroup(selectedGroup) {
  const leaderKeys = ['nguyenconghoan', 'nguyenminhcuong', 'nguyentrungkien'];
  const allSheetUsersSet = new Set();
  const teamLeaderName = findTeamLeaderName(selectedGroup);
  const teamMembersSet = new Set();

  // 1. Extract ALL users from appState.users (Sheet User)
  if (appState.users && Array.isArray(appState.users)) {
    appState.users.forEach(u => {
      const { name, group } = getUserNameAndGroup(u);
      ...
      allSheetUsersSet.add(trimmed);

      if (selectedGroup && isGroupMatch(group, selectedGroup)) {
        teamMembersSet.add(trimmed);
      }
    });
  }

  // Backup fallback if appState.users is empty or not loaded
  if (allSheetUsersSet.size === 0) {
    if (appState.tovien && Array.isArray(appState.tovien)) {
      appState.tovien.forEach(row => {
        const info = getTovienRowInfo(row);
        if (info.leaderName && !leaderKeys.includes(cleanKey(info.leaderName))) allSheetUsersSet.add(info.leaderName.trim());
        if (info.empName && !leaderKeys.includes(cleanKey(info.empName))) allSheetUsersSet.add(info.empName.trim());
        if (selectedGroup && isGroupMatch(info.group, selectedGroup)) {
          if (info.leaderName) teamMembersSet.add(info.leaderName.trim());
          if (info.empName) teamMembersSet.add(info.empName.trim());
        }
      });
    }
    if (appState.tasks && Array.isArray(appState.tasks)) {
      appState.tasks.forEach(t => {
        ...
        const rName = getTaskEmpRName(t) || t['Tên NV (R)'];
        const cName = getTaskEmpCName(t) || t['Tên NV (C)'];
        if (aName && !leaderKeys.includes(cleanKey(aName))) allSheetUsersSet.add(String(aName).trim());
        if (rName && !leaderKeys.includes(cleanKey(rName))) allSheetUsersSet.add(String(rName).trim());
        if (cName && !leaderKeys.includes(cleanKey(cName))) allSheetUsersSet.add(String(cName).trim());
      });
    }
  }
  ...
  // Priority 1: Team Leader for selected group
  if (teamLeaderName && !result.includes(teamLeaderName)) {
    result.push(teamLeaderName);
  }

  // Priority 2: Members of selected group
  Array.from(teamMembersSet).sort().forEach(m => {
    if (!result.includes(m)) result.push(m);
  });

  // Priority 3: All other users from Sheet User (ensures entire list in Sheet User is available to pick)
  Array.from(allSheetUsersSet).sort().forEach(u => {
...
function populateSelects() {
  ...
  appState.dropdownData.leadera = getEmployeesByGroup(chuTriGrp);
  appState.dropdownData.chutri = getEmployeesByGroup(effectiveCollabGrp);
  appState.dropdownData.phoihop = getEmployeesByGroup(effectiveCollabGrp);

  updateGlobalUserSelectOptions();
}

function handleModalGroupChange(selectedGroup) {
  const leaderAUsers = getEmployeesByGroup(selectedGroup);
  const leadersList = ['Nguyễn Công Hoan', 'Nguyễn Minh Cường', 'Nguyễn Trung Kiên'];
  ...
  const collabGroup = toCollabSelect ? toCollabSelect.value : '';
  const chuTriUsers = getEmployeesByGroup(collabGroup || selectedGroup);
...
function handleModalCollabGroupChange(selectedGroup) {
  ...
  const collabUsers = getEmployeesByGroup(effectiveGrp);
...
function handleGlobalFilter() {
  ...
  appState.filters.collabGroup = cgEl ? cgEl.value : '';
  appState.filters.leaderA = lEl ? lEl.value : '';
  appState.filters.user = uEl ? uEl.value : '';
  appState.filters.collabUser = cuEl ? cuEl.value : '';
  ...
  if (appState.tasks && Array.isArray(appState.tasks)) {
    appState.tasks.forEach(t => {
      ...
      if (id) {
        ...
        const titleKey = cleanKey(t['Tiêu đề'] || t['Tiêu đề công việc'] || '');
        if (titleKey) taskMap.set('task_' + titleKey, t);
        else taskMap.set('task_' + Math.random(), t);
      }
    });
  }
  ...
  if (appState.totruonggiaoviec && Array.isArray(appState.totruonggiaoviec)) {
    appState.totruonggiaoviec.forEach(t => {
      ...
      if (id && !taskMap.has(id)) {
        ...
        const titleKey = cleanKey(t['Tiêu đề'] || t['Tiêu đề công việc'] || '');
        if (titleKey && !taskMap.has('totruong_' + titleKey)) {
          taskMap.set('totruong_' + titleKey, t);
        } else if (!titleKey) {
          taskMap.set('totruong_' + Math.random(), t);
        }
      }
    });
  }
...
function resetAllGlobalFilters() {
  ...
  const sEl = document.getElementById('global-search-input');
  if (sEl) sEl.value = '';
  ['group', 'collabGroup', 'leaderA', 'user', 'collabUser'].forEach(type => {
    const hiddenSelectId = {
      group: 'global-group-select',
      collabGroup: 'global-collab-group-select',
      leaderA: 'global-leader-select',
...
function renderTaskListTable() {
  ...
  tbody.innerHTML = '';
  if (filtered.length === 0) {
    if (!appState.isGAS && !appState.apiUrl && (!appState.tasks || appState.tasks.length === 0)) {
      tbody.innerHTML = `<tr><td colspan="17" style="text-align:center; padding:35px; color:#f59e0b;"><i class="fa-solid fa-triangle-exclamation" style="margin-right:8px;"></i> Chưa cấu hình URL Google Apps Script. Vui lòng bấm biểu tượng <strong>Cấu hình (⚙️)</strong> ở góc trên bên phải để dán Web App URL và tải công việc. <button class="btn-dark-sec btn-sm" onclick="openModal('modal-settings')" style="margin-left:12px;"><i class="fa-solid fa-gear"></i> Mở cấu hình</button></td></tr>`;
    } else if (!appState.tasks || appState.tasks.length === 0) {
      tbody.innerHTML = `<tr><td colspan="17" style="text-align:center; padding:35px; color:var(--text-muted);"><i class="fa-solid fa-arrows-rotate fa-spin" style="margin-right:8px;"></i> Đang tải danh sách công việc... <button class="btn-dark-sec btn-sm" onclick="appState.loadData(true, true)" style="margin-left:12px;"><i class="fa-solid fa-rotate-right"></i> Tải lại</button></td></tr>`;
    } else {
      tbody.innerHTML = `<tr><td colspan="17" style="text-align:center; padding:35px; color:var(--text-muted);"><i class="fa-solid fa-filter-circle-xmark" style="margin-right:8px;"></i> Không tìm thấy công việc nào phù hợp với bộ lọc hiện tại. <button class="btn-dark-sec btn-sm" onclick="resetAllGlobalFilters()" style="margin-left:12px;"><i class="fa-solid fa-xmark"></i> Xóa bộ lọc</button></td></tr>`;
    }
    return;
  }
