...
function getToTruongTasks() {
  const source = (appState.totruonggiaoviec && Array.isArray(appState.totruonggiaoviec) && appState.totruonggiaoviec.length > 0)
    ? appState.totruonggiaoviec
    : appState.tasks;
  ...
  return source.map(t => {
    ...
    const leaderName = getTaskLeaderName(t) || getTaskAssignee(t);
    const leaderCode = getTaskLeaderCode(t);

    const empRName = getTaskEmpRName(t) || (getTaskAssignee(t) !== leaderName ? getTaskAssignee(t) : '');
