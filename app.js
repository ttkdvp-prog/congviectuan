...
function getTaskLeaderName(task) {
  ...
  for (let k in task) {
    const ck = cleanKey(k);
    if (ck === 'tentotruong' || ck === 'totruong' || ck === 'nguoigiaoviec' || ck === 'tennguoigiaoviec') {
      if (task[k] !== undefined && task[k] !== null && String(task[k]).trim() !== '') {
        return String(task[k]).trim();
      }
    }
  ...
  for (let k in task) {
    const ck = cleanKey(k);
    if (ck.includes('totruong') && !ck.includes('ma')) {
      if (task[k] !== undefined && task[k] !== null && String(task[k]).trim() !== '') {
        return String(task[k]).trim();
      }
    }
  }
  return task['Tên tổ trưởng'] || '';
}
...
function getTaskLeaderCode(task) {
  ...
  for (let k in task) {
    const ck = cleanKey(k);
    if (ck === 'manvtotruong' || ck === 'matotruong' || ck === 'manguoigiaoviec') {
      if (task[k] !== undefined && task[k] !== null && String(task[k]).trim() !== '') {
        return String(task[k]).trim();
      }
    }
  }
  return task['Mã NV tổ trưởng'] || '';
}
  ...
  let filtered = toTruongTasks.filter(t => {
    ...
    if (usrClean) {
      ...
      if (isLeader) {
        ...
        const phoiHopClean = cleanKey(phoiHop);

        const matchChuTri = chuTriClean && (chuTriClean.includes(usrClean) || usrClean.includes(chuTriClean));
        const matchPhoiHop = phoiHopClean && (phoiHopClean.includes(usrClean) || usrClean.includes(phoiHopClean));
